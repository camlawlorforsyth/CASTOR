...
def all_metrics(fast_file) :
    ...
    for i, (subID, snap) in enumerate(zip(subIDs[mech_mask][idx:idx+1], snaps[mech_mask][idx:idx+1])) :
        ...
        CSFs[i] = calculate_C_SF(lsfr, kpc_pix, Re_pix, bin_edges)
        ...
        RSFs[i] = calculate_R_SF(subID) # to implement
...
def calculate_metrics(subID, lsfr, bin_centers, profile) :
    return np.array([calculate_C_SF(lsfr), calculate_R_SF(subID),
                     calculate_Rinner(bin_centers, profile),
                     calculate_Router(bin_centers, profile)])

def calculate_C_SF(lsfr, kpc_pix, Re_pix, bin_edges) :
    
    # 1 kpc = ~3.19 pix at redshift z = 0.5
    
    # calculate the mass formed within 100 Myr, based on the SFR from FAST++
    young_mass_profile = np.power(10, lsfr[:-2])*1e8
    
    # define the fraction of the young mass profile that we want to use to
    # calculate C_SF, based on the inner 1 kpc
    mass_frac = np.full(lsfr[:-2].shape, 0.0)
    ...
    C_SF = np.sum(mass_frac*young_mass_profile)/np.sum(young_mass_profile)
    
    return C_SF

def calculate_R_SF(subID) :
...
def calculate_C_SF_alt2(lsfr, kpc_pix, Re_pix, bin_edges) :
    
    young_mass_cumulative_profile = np.nancumsum(np.power(10, lsfr[:-2])*1e8)
...
def calculate_C_SF_alt1(lsfr) :
    return np.power(10, lsfr[20])/np.power(10, lsfr[21])

def calculate_R_SF_alt1(subID) :
    
    data_uv, shape, _, _, _, _, plate_scale = open_cutout(
        'cutouts/quenched/{}/castor_uv_ultradeep.fits'.format(subID))
    
    cent = int((shape[0] - 1)/2)
    center = (cent, cent)
    
    cog_uv = CurveOfGrowth(data_uv, center, np.arange(0.5, shape[0]/2 + 0.5, 0.5))
    cog_uv.normalize('max')
    Re_uv = cog_uv.calc_radius_at_ee(0.5) # in pixels
    
    data_f158, _ = open_cutout(
        'cutouts/quenched/{}/roman_f158_hlwas.fits'.format(subID), simple=True)
    
    cog_f158 = CurveOfGrowth(data_f158, center, np.arange(0.5, shape[0]/2 + 0.5, 0.5))
    cog_f158.normalize('max')
    Re_f158 = cog_f158.calc_radius_at_ee(0.5) # in pixels
    
    return np.log10(Re_uv/Re_f158)
